...
function renderTopPapers(papers) {
  ...
  papers.forEach((paper, i) => {
    ...
    div.append("p").attr("class", "font-bold").text(`${i + 1}. ${paper.title}`);
    div.append("p").attr("class", "text-sm text-gray-600").text(`Citations: ${paper.citationCount}`);
  });
}
...
function renderAuthorChart(authors) {
  ...
  const svg = d3.select("#yearly-top-authors").html("")
    .append("svg")
    .attr("width", width)
    .attr("height", height);
...
function renderSankeyDiagram(data, authorColorMap) {
  const svg = d3.select("#yearly-sankey").html("")
    .append("svg")
    .attr("width", 700)
    .attr("height", 500);
  ...
  svg.append("g")
    ...
    .attr("opacity", 0.5)
    .append("title")
    .text(d => `${nodes[d.source.index].name.replace(/^A:/, "")} → ${nodes[d.target.index].name.replace(/^V:/, "")}: ${d.value}`);
}

function loadYearlyData(year, observer) {
  d3.csv(`data/trending/${year}.csv`).then(data => {
    const sortedPapers = data.filter(d => d.title && d.citationCount)
